...
import { Smartphone, Zap, FileCheck, Wallet } from 'lucide-react';
import ContactSection from '../components/ContactSection';
import LegalWarning from '../components/LegalWarning';
import { getFaqData } from '../data/help/faq';
import { HELP_CATEGORIES } from '../data/help/config';
      ...
      <div className="max-w-7xl mx-auto px-6 py-20 space-y-40">
        ...
        <section className="space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { icon: <Smartphone className="text-brand-primary" />, step: "01", title: "Demande en ligne", desc: "Formulaire express en moins de 3 minutes." },
              { icon: <Zap className="text-amber-500" />, step: "02", title: "Accord immédiat", desc: "Réponse de principe instantanée par algorithme." },
              { icon: <FileCheck className="text-emerald-500" />, step: "03", title: "Signature eIDAS", desc: "Signez numériquement et déposez vos pièces." },
              { icon: <Wallet className="text-indigo-500" />, step: "04", title: "Transfert 48h", desc: "Fonds versés sur votre compte habituel." }
            ].map((item, i) => (
              <FadeIn key={i} delay={i * 0.1}>
                <div className="p-10 bg-slate-50 dark:bg-slate-900/50 rounded-[3.5rem] border border-slate-100 dark:border-slate-800 relative group hover:border-brand-primary/20 transition-all">
                  <span className="absolute top-8 right-10 text-5xl font-black italic text-slate-200 dark:text-slate-800 group-hover:text-brand-primary/10 transition-colors">{item.step}</span>
                  <div className="w-14 h-14 bg-white dark:bg-slate-800 rounded-2xl flex items-center justify-center shadow-sm mb-10 group-hover:scale-110 transition-transform">{item.icon}</div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
            ...
            <div className="flex items-center space-x-2 overflow-x-auto scrollbar-hide -mx-6 px-6 pb-2 md:pb-0 md:mx-0">
              {HELP_CATEGORIES.map((cat) => (
                ...
                >
                  {cat.icon}
                  <span>{cat.translationKey ? t(cat.translationKey) : cat.title}</span>
                </button>
              ))}
            </div>
          </div>
