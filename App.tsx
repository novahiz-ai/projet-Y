              ...
              {!isSearchOpen ? (
                <motion.div key="nav-content" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className={`flex-1 flex items-center justify-between h-full ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`flex items-center space-x-2 ${isRTL ? 'space-x-reverse' : ''}`}>
                    <button onClick={() => setIsSearchOpen(true)} className="p-2 lg:p-2.5 rounded-xl hover:bg-black/5 dark:hover:bg-white/10 transition-colors text-slate-600 dark:text-slate-300" aria-label="Rechercher">
                      <Search size={18} />
                    </button>
