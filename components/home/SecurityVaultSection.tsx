...
const SecurityVaultSection: React.FC = () => {
  ...
  return (
    <section className="relative py-24 lg:py-40 bg-slate-950 overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_0%,#10b98122,transparent_50%)]"></div>
      
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center">
          <div className="space-y-10">
            <FadeIn direction="right">
              <div className="inline-flex items-center space-x-3 bg-white/5 text-brand-green px-4 py-1.5 rounded-full border border-white/10">
                <ShieldCheck size={14} />
              ...
              <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter italic leading-[0.95] text-white mt-6">
                VOTRE SÉCURITÉ, <br />
                <span className="text-brand-green">NOTRE PRIORITÉ.</span>
              </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
              {securityFeatures.map((feat, i) => (
                <FadeIn key={i} delay={i * 0.1}>
                  <div className="p-6 bg-white/5 rounded-3xl border border-white/5 hover:border-brand-green/30 transition-all group">
                    <div className="w-12 h-12 bg-brand-green/20 text-brand-green rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                      {feat.icon}
                    </div>
