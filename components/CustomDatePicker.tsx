...
const CustomDatePicker: React.FC<CustomDatePickerProps> = ({ value, onChange, label, error }) => {
  ...
  const renderDays = () => {
    ...
    const startOffset = (firstDayOfMonth(currentYear, currentMonth) + 6) % 7;
  ...
  return (
    <div className="space-y-1 relative">
      ...
      <button
        ...
        className={`w-full bg-slate-50/50 dark:bg-slate-900/30 border rounded-2xl py-4 px-6 font-bold text-lg text-left transition-all outline-none flex items-center justify-between shadow-inner-soft ${
          error ? 'border-rose-500 ring-4 ring-rose-500/10' : isOpen ? 'border-brand-primary/40 ring-4 ring-brand-primary/5' : 'border-slate-200 dark:border-slate-800'
            ...
            >
              <div className="p-6 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                ...
                <div className="flex space-x-2">
                  <button type="button" onClick={() => setViewMode(viewMode === 'months' ? 'days' : 'months')} className="px-4 py-2 hover:bg-white dark:hover:bg-slate-800 rounded-xl text-xs font-black uppercase tracking-widest text-slate-900 dark:text-white">
                    {months[currentMonth]}
                  </button>
                  <button type="button" onClick={() => setViewMode(viewMode === 'years' ? 'days' : 'years')} className="px-4 py-2 hover:bg-white dark:hover:bg-slate-800 rounded-xl text-xs font-black uppercase tracking-widest text-slate-900 dark:text-white">
                    {currentYear}
                  </button>
                </div>
              ...
              <div className="p-6">
                ...
                {viewMode === 'months' && (
                  <div className="grid grid-cols-3 gap-3">
                    {months.map((m, i) => (
                      <button key={m} type="button" onClick={() => { setViewDate(new Date(currentYear, i, 1)); setViewMode('days'); }} className={`py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all ${i === currentMonth ? 'bg-brand-primary text-white shadow-brand' : 'bg-slate-50 dark:bg-slate-900 text-slate-500 hover:bg-slate-100'}`}>
                        {m}
                      </button>
                    ))}
                  </div>
              ...
              <div className="p-6 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <button type="button" onClick={() => { setViewDate(new Date()); setViewMode('days'); }} className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-brand-primary">Aujourd'hui</button>
                <button type="button" onClick={() => setIsOpen(false)} className="bg-brand-primary text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-brand flex items-center space-x-2"><Check size={14} /><span>Fermer</span></button>
              </div>
            </motion.div>
          </div>
        )}
